...
struct DataCacheConfiguration {
    ...
    private let maxCacheAge = (60 * 60 * 24 * 7) as TimeInterval
...
class DataCache {
    ...
    private func getFilesToRemove() -> [URL] {
        ...
        fileURLs.forEach {
            (aFileURL) -> () in
            let values: [URLResourceKey : AnyObject]? = try? (aFileURL as NSURL).resourceValues(forKeys: properties)
            if let values = values, let modificationDate = values[URLResourceKey.contentModificationDateKey] as? Date {
                if (modificationDate as NSDate).laterDate(expirationDate) == modificationDate {
                    ...
                    if let fileSize = values[URLResourceKey.totalFileAllocatedSizeKey] as? UInt {
                        cacheSize += fileSize
                    }
                } else {
    ...
    private func sortFileURLByModificationDate(_ urls: [URL]) -> [URL] {
        return urls.sorted {
            (url1, url2) -> Bool in
